...
def choose(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    true_paragraphs = [paragragh for paragragh in paragraphs if select(paragragh)]
...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    def about_topic(paragragh):
        for word in topic:
            if word in split(lower(remove_punctuation(paragragh))):
                return True
        return False
    return about_topic
    # END PROBLEM 2
...
def feline_fixes(start, goal, limit):
    ...
    # assert False, 'Remove this line'
    if start is goal: # Fill in the condition
        # BEGIN
        "*** YOUR CODE HERE ***"
        return 0
        # END
...
def time_per_word(times_per_player, words):
    ...
    for i in range(len(times_per_player)):
        new = [1] * (len(times_per_player[i]) - 1)
        for j in range(1, len(times_per_player[i])):
            new[j - 1] = times_per_player[i][j] - times_per_player[i][j - 1]
        times.append(new)
